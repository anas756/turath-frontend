...
const isVideoFile = (file) =>
  file?.type?.toString().toLowerCase() === 'video' || file?.mime_type?.startsWith('video/');
...
const m = {
  ...
  val: { fontSize: '0.82rem', color: 'var(--on-surface)', fontWeight: 600, textAlign: 'right' },
  ...
  fileMeta: { display: 'block', fontSize: '0.68rem', color: 'var(--on-surface-muted)', marginTop: '0.2rem' },
  footer: {
    display: 'flex',
    justifyContent: 'space-between',
    alignItems: 'center',
...
export default function ShowMediaDetails({ media, onClose, onEdit }) {
  ...
  const tags = Array.isArray(media.tags)
    ? media.tags
    : (media.tags ? media.tags.split(',').map((tag) => tag.trim()).filter(Boolean) : []);
  ...
  return (
    <div style={m.container}>
      <div style={m.header}>
        ...
        <button type="button" onClick={onClose} style={m.closeBtn}>x</button>
      </div>
      ...
      <div style={m.grid}>
        <div style={m.card}>
          <p style={m.secLabel}>File Summary</p>
          <div style={m.list}>
            <div style={m.row}>
              <span style={m.lbl}>Files</span>
              <Val value={`${files.length} ${files.length === 1 ? 'file' : 'files'}`} />
            </div>
            <div style={m.row}>
              <span style={m.lbl}>First format</span>
              <Val value={media.format?.toUpperCase()} />
            </div>
            <div style={m.row}>
              <span style={m.lbl}>First resolution</span>
              <Val value={media.resolution} />
            </div>
            <div style={m.rowLast}>
              <span style={m.lbl}>Total size</span>
              <Val value={fileSize} />
            </div>
          </div>
        </div>
      ...
      <div style={m.cardFull}>
        <p style={m.secLabel}>Files</p>
        {files.length > 0 ? (
          <div style={m.filesGrid}>
            {files.map((file) => (
              <div style={m.fileCard} key={file.id || file.path}>
                {isVideoFile(file) ? (
                  file.thumbnail_url ? (
                    <img src={file.thumbnail_url} alt="" style={m.filePreview} />
                  ) : (
                    <video src={file.url} muted playsInline preload="metadata" style={m.filePreview} />
                  )
                ) : (
                  <img src={file.url} alt="" style={m.filePreview} />
                )}
                <span style={m.fileName}>{file.original_name || file.path}</span>
                <span style={m.fileMeta}>
                  {[file.type, file.format, formatSize(file.size)].filter(Boolean).join(' - ')}
                </span>
              </div>
            ))}
          </div>
        ) : (
          <span style={m.descMuted}>No files attached</span>
        )}
      </div>
      ...
      <div style={m.cardFull}>
        ...
        {tags.length > 0
          ? <div style={m.tagsRow}>{tags.map((tag) => <span key={tag} style={m.tag}>{tag}</span>)}</div>
      ...
      <div style={m.footer}>
        <span style={m.footerMeta}>
          {[addedAt ? `Added ${addedAt}` : '', curatorName].filter(Boolean).join(' - ')}
        </span>
        {onEdit && (
